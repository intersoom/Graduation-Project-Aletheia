...
function WaveScene2() {
  ...
  useEffect(() => {
    ...
    // camera.position.x = 1.1096;
    // camera.position.y = 9.66;
    ...
    function createGeometry() {
      ...
      let heightMap = new Array(rows + 1);
      let vecCount = 0;
    ...
    const particleTexture = textureLoader.load("smoke.png");

    const geo = createGeometry();
    const geo2 = createGeometry();

    const pointCloud = new THREE.Mesh(
      geo,
    ...
    pointCloud.translateZ(-80);

    const pointCloud2 = new THREE.Mesh(
      geo2,
      new THREE.MeshBasicMaterial({
        color: new THREE.Color("rgb(240,250,0)"),
      })
    );
    pointCloud2.rotation.y = -Math.PI / 2;
    scene.add(pointCloud2);
    // pointCloud2.translateY(-25);
    pointCloud2.translateZ(-80);
    ...
    function perlinAnimate() {
      ...
      for (let z = 0; z < rows; z++) {
        for (let x = 0; x < cols; x++) {
          ...
          pointCloud.geometry.attributes.position.array[i] =
            // simplex2(pX, pZ) * waveHeight + y;
            simplex2(pX, pZ) * waveHeight;
          i += 3;
          // }
        }
      }
      ...
      for (let z = 0; z < rows2; z++) {
        for (let x = 0; x < cols2; x++) {
          for (let y = 0; y < verticesHeight2; y++) {
            ...
            pointCloud2.geometry.attributes.position.array[i] =
              -simplex2(pX2, pZ2) * waveHeight2 - y;
            i += 3;
          }
        }
      }
      pointCloud.geometry.getAttribute("position").needsUpdate = true;
      pointCloud2.geometry.getAttribute("position").needsUpdate = true;
    }
